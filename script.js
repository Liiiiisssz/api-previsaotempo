...
async function previsaoJaragua(){
    datas();
    try{
        const dados = await fetch(
            `https://api.open-meteo.com/v1/forecast?latitude=-26.4861&longitude=-49.0667&current_weather=true&hourly=temperature_2m,relative_humidity_2m,windspeed_10m,weathercode&daily=weathercode,temperature_2m_max,temperature_2m_min&forecast_days=4&timezone=auto`
        )
        const climaData = await dados.json();
        ...
        trocarFundo(hora);
        textoEicons(hora, codTempo, codTempoD1, codTempoD2, codTempoD3);
...
function textoEicons(hora, codTempo, codTempoD1, codTempoD2, codTempoD3){
    switch(codTempo){
        case 0: case 1:
...
function trocarFundo(hora){
    const img = document.getElementById("body");
    const pesquisa = document.getElementById("pesquisa");
    const principal = document.getElementById("bg-principal");
    const botao = document.getElementById("botao");
    const um = document.getElementById("dia-um");
    const dois = document.getElementById("dia-dois");
    const tres = document.getElementById("dia-tres");

    if(!hora){
        img.classList.replace("bg-[url(/assets/ceu.jpg)]", "bg-[url(/assets/noite.png)]");
        pesquisa.classList.replace("bg-sky-200/50", "bg-slate-600/50");
        principal.classList.replace("bg-sky-200/50", "bg-slate-600/50");
        botao.classList.replace("bg-sky-400", "bg-slate-600");
        um.classList.replace("bg-sky-300/50", "bg-slate-500/50");
        dois.classList.replace("bg-sky-300/50", "bg-slate-500/50");
        tres.classList.replace("bg-sky-300/50", "bg-slate-500/50");
    } else {
        img.classList.replace("bg-[url(/assets/noite.png)]", "bg-[url(/assets/ceu.jpg)]");
        pesquisa.classList.replace("bg-slate-600/50", "bg-sky-200/50");
        principal.classList.replace("bg-slate-600/50", "bg-sky-200/50");
        botao.classList.replace("bg-slate-600", "bg-sky-400");
        um.classList.replace("bg-slate-500/50", "bg-sky-300/50");
        dois.classList.replace("bg-slate-500/50", "bg-sky-300/50");
        tres.classList.replace("bg-slate-500/50", "bg-sky-300/50");
    }
}